...
import { getProjectStageList } from 'redux/slices/krowd_slices/stage';
import { Project } from '../../../@types/krowd/project';
import HeaderBreadcrumbs from 'components/HeaderBreadcrumbs';
import { PATH_DASHBOARD } from 'routes/paths';
import { fCurrency2, fCurrency } from 'utils/formatNumber';
...
export default function KrowdProjectStage({ project }: { project: Project }) {
  ...
  const { listOfChartStage, isLoading } = useSelector((state: RootState) => state.stage);
      ...
      y: {
        formatter: (y: number) => {
          if (typeof y !== 'undefined') {
            return `${fCurrency(y.toFixed(0))} VND`;
          }
          return y;
        }
      }
