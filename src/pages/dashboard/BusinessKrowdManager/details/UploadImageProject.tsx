...
export default function UploadImageProject({ project: p, closeDialog }: AccountGeneralProps) {
  ...
  const formikImage = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      try {
        setSubmitting(true);
        await UploadAPI.postImagePicture({ id: p.id, file: fileUpload })
          .then(() => {
  ...
  return (
    <FormikProvider value={formikImage}>
      <Form noValidate autoComplete="off" onSubmit={handleSubmitImage}>
        ...
        {fileUpload && (
          <Box display="flex" my={3} gap={2} justifyContent="flex-end" pr={5}>
