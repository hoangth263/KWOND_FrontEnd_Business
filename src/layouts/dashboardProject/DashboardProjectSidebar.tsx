...
export default function DashboardProjectSidebar({
  ...
  const renderContent = (
    ...
    >
      ...
      >
        ...
        {isCollapse ? (
          ...
          <Link underline="none" component={RouterLink} to={PATH_DASHBOARD_PROJECT.project.root}>
            <AccountStyle>
              <Avatar src={businessProjectDetail.projectDetail?.image ?? ''} alt={''}></Avatar>
              <Box sx={{ ml: 2 }}>
                <Typography variant="subtitle2" sx={{ color: 'text.primary' }}>
                  {businessProjectDetail.projectDetail?.name ?? ''}
                </Typography>
              </Box>
            </AccountStyle>
          </Link>
      ...
      <NavSection navConfig={SidebarProjectConfig} isShow={!isCollapse} accessRole={user?.role} />
      ...
      {!isCollapse && (
        ...
        >
          <Typography gutterBottom variant="subtitle1">
            <Link href={PATH_DASHBOARD.projects.PoM_project_Krowd}>Quay lại bảng dự án</Link>
          </Typography>
        </Stack>
      )}
    </Scrollbar>
